...
def tex2pdf(tex_file):
    logger.debug('Executing pdflatex on {}.'.format(tex_file))
    if not tex_file.endswith('.tex'):
        logger.error('The argument tex_file={} passed to tex2pdf is not a .tex file.'.format(tex_file))
        exit(1)
...
def generate_problemset_pdf(problems, frontpage, tex_dir, params):
    ...
    for problem in problems:
        ...
        if not os.path.isfile(maybe_tex):
            logger.warning('The tex source {} does not exist, but it is required to generate the pdf with all the problems.'
                           .format(maybe_tex))
            continue
...
def generate_solutions_pdf(problems, frontpage, tex_dir, params):
    ...
    for problem in problems:
        ...
        if not os.path.isfile(maybe_tex):
            logger.warning('The tex source {} does not exist, but it is required to generate the pdf with all the solutions.'
                           .format(maybe_tex))
            continue
